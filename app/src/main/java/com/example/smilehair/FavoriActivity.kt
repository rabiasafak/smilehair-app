...
class FavoriActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val textView = findViewById<TextView>(R.id.favoriEkimiNedir)
        textView.text = Html.fromHtml(
            getString(R.string.favori_ekimi),
            Html.FROM_HTML_MODE_LEGACY
        )
        val textView2 = findViewById<TextView>(R.id.favoriYanEtkiler)
        textView2.text = Html.fromHtml(
            getString(R.string.favori_ekimi_yan_etkiler),
            Html.FROM_HTML_MODE_LEGACY
        )
        val textView3 = findViewById<TextView>(R.id.favoriSonrasiBakim)
        textView3.text = Html.fromHtml(
            getString(R.string.favori_ekimi_sonrasi),
            Html.FROM_HTML_MODE_LEGACY
        )
        val textView4 = findViewById<TextView>(R.id.favoriDokulmesiNede)
        textView4.text = Html.fromHtml(
            getString(R.string.favori_dokulme_nedenleri),
            Html.FROM_HTML_MODE_LEGACY
        )
        val textView5 = findViewById<TextView>(R.id.favoriEkimiIcinHazirlanin)
        textView5.text = Html.fromHtml(
            getString(R.string.favori_ekimi_oncesi),
            Html.FROM_HTML_MODE_LEGACY
        )
        val textView6 = findViewById<TextView>(R.id.herkesFavoriEkimYaptirabilirMi)
        textView6.text = Html.fromHtml(
            getString(R.string.favori_ekimi_kimler_icin),
            Html.FROM_HTML_MODE_LEGACY
        )
